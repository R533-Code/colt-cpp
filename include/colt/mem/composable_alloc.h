...
namespace clt::mem
{
      ...
      , private Fallback
  {
    ...
    constexpr MemBlock alloc(u64 size) noexcept
    {
  ...
  template<u64 SIZE, meta::Allocator Primary, meta::Allocator Secondary>
  ...
  private:
    ...
    static constexpr MemBlock from_ret_to_prefix(MemBlock blk) noexcept
    {
      return MemBlock{
          ...
          blk.size() + prefix_size + suffix_size};
    }
  ...
  public:
    ...
    constexpr MemBlock alloc(u64 size) noexcept
    {
      if (auto blk = allocator::alloc(prefix_size + size + suffix_size))
        ...
        noexcept(std::is_nothrow_constructible_v<Suffix, Args...>)
    {
      ...
      return *static_cast<Suffix*>(
          new (static_cast<u8*>(blk.ptr()) + blk.size())
        ...
        noexcept(std::is_nothrow_destructible_v<Suffix>)
    {
      ...
      static_cast<Suffix*>(static_cast<u8*>(blk.ptr()) + blk.size())
          ->~Suffix();
    }
    ...
    constexpr helper_suffix_t& get_suffix(MemBlock blk) const noexcept
    {
      ...
      static_assert(
          !std::same_as<void, Suffix>, "AffixAllocator does not have a suffix!");
      return *static_cast<Suffix*>(
          static_cast<void*>(static_cast<u8*>(blk.ptr()) + blk.size()));
    }
  ...
  template<meta::Allocator allocator, u64 BUFFER_SIZE, u8 PATTERN = 0xFD>
  ...
  class MemCorruptDetector
      : private AffixAllocator<
            allocator, std::array<u8, BUFFER_SIZE>,
            std::array<u8, BUFFER_SIZE>>
  {
    ...
    using Array = std::array<u8, BUFFER_SIZE>;
  ...
  public:
    ...
    {
      return std::memcmp(
                 &Allocator::get_prefix(blk), pattern.data(), BUFFER_SIZE)
             != 0;
    }
    ...
    {
      return std::memcmp(
                 &Allocator::get_suffix(blk), pattern.data(), BUFFER_SIZE)
             != 0;
    }
  ...
  public:
    ...
    /// @param size The size of the allocation
    /// @return Allocated MemBlock or empty MemBlock
    constexpr void* alloc(u64 size) noexcept
    {
      ...
      if (auto blk = Allocator::alloc(size))
      {
        Allocator::create_prefix(blk, size);
        if constexpr (is_debug_build())
          Allocator::create_suffix(blk, size);
        return blk.ptr();
      }
      return nullptr;
    }

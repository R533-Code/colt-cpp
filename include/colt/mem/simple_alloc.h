...
namespace clt::mem
{
  ...
  public:
    ...
    /// @return nullblk
    constexpr MemBlock alloc(u64) const noexcept { return nullblk; }
    ...
    /// @return false
    constexpr bool expand(MemBlock& blk, u64) const noexcept
    {
      assert_true("NULLAllocator must only receive empty blocks!", blk == nullblk);
      return false;
    }
    ...
    /// @param  Size for expansion
    /// @return false
    constexpr bool realloc(MemBlock& blk, u64) const noexcept
    {
      assert_true("NULLAllocator must only receive empty blocks!", blk == nullblk);
      return false;
    }
  ...
  public:
    ...
    /// @return nullblk
    MemBlock alloc(u64 size) const noexcept
    {
      assert_true("Mallocator must not allocate block of size '0'!", size != 0);
    ...
    bool realloc(MemBlock& blk, u64 sz) const noexcept
    {
      auto ptr = std::realloc(blk.ptr(), sz);
      if (ptr == nullptr)
        return false;
      blk = MemBlock{ptr, sz};
      return true;
  ...
  public:
    ...
    constexpr MemBlock alloc(u64 sze) noexcept
    {
      size_t aligned_size = round_to_alignment<ALIGN>(sze);
    ...
    constexpr void dealloc(MemBlock to_free) noexcept
    {
      ...
      size_t aligned_size = round_to_alignment<ALIGN>(to_free.size());
    ...
    /// @param n The new size of the block
    /// @return True if reallocation was successful
    constexpr bool realloc(MemBlock& blk, u64 n) noexcept
    {
      ...
      if (static_cast<u8*>(blk.ptr()) + blk.size() == top)
      {
        if (can_allocate(n - blk.size()))
        {
          top += round_to_alignment<ALIGN>(n - blk.size());
          blk = MemBlock{blk.ptr(), n};
          return true;
        }
      ...
      if (nblk.is_null())
        return false;
      std::memcpy(nblk.ptr(), blk.ptr(), blk.size());
    ...
    constexpr bool expand(MemBlock& blk, u64 delta) noexcept
    {
      ...
      if (can_allocate(delta - blk.size()))
      {
        top += round_to_alignment<ALIGN>(delta - blk.size());
        blk = MemBlock{blk.ptr(), delta};
        return true;
      }
      return false; //not enough memory
    }
  ...
  template<
      meta::Allocator allocator, u64 LOWER, u64 UPPER,
      size_t MAX_SAVED = 16>
    requires(LOWER >= 16) && (UPPER >= LOWER) && (MAX_SAVED != 0)
  ...
  public:
    ...
    /// @return Allocated MemBlock or an empty MemBlock on failure
    constexpr MemBlock alloc(u64 n) noexcept
    {
      if (root != nullptr && is_in_range(n))
      {
        if (auto find = get_node_of_size(n); find)
          return {find, n};
      }
    ...
    constexpr void dealloc(MemBlock blk) noexcept
    {
      ...
      if (!is_in_range(blk.size()) || saved_count == MAX_SAVED)
      {
      ...
      root = new (blk.ptr()) Node{blk.size(), root};
      //Add to save count
      ++saved_count;
    }

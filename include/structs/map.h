...
namespace clt
{
  ...
  public:    
    ...
    constexpr const Slot* find(meta::copy_trivial_t<const Key&> key) const noexcept
    {
    ...
    /// @param key The key to search for
    /// @return Pointer to the key/value pair if found, or null
    constexpr Slot* find(meta::copy_trivial_t<const Key&> key) noexcept
    {
    ...
    constexpr bool contains(meta::copy_trivial_t<const Key&> key) const noexcept
    {
    ...
    constexpr std::pair<Slot*, InsertionResult> insert(meta::copy_trivial_t<const Key&> key, meta::copy_trivial_t<const Value&> value)
      noexcept(std::is_nothrow_destructible_v<Key>
        && std::is_nothrow_destructible_v<Value>
    ...
    constexpr bool erase(meta::copy_trivial_t<const Key&> key)
      noexcept(std::is_nothrow_destructible_v<Key>
        && std::is_nothrow_destructible_v<Value>)
    {
    ...
    constexpr const Slot* operator[](meta::copy_trivial_t<const Key&> key) const noexcept
    {
      return find(key);
    }
    ...
    /// @param key The key to search for
    /// @return Pointer to the found slot or null if not found
    constexpr Slot* operator[](meta::copy_trivial_t<const Key&> key) noexcept
    {
      return find(key);
    }
  ...
  private:
    ...
    static constexpr bool find_key(size_t key_hash, meta::copy_trivial_t<const Key&> key, size_t& prob,
      const Vector<details::KeySentinel>& metadata, mem::TypedBlock<Slot> blk) noexcept
    {
      assert_true("Wrong use of API", key_hash == GetHash(key));

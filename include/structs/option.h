...
namespace clt
{
  ...
  public:
    ...
    constexpr Optional(meta::NoneT) noexcept
      : is_none_v(true) {}

    /// @brief Copy constructs an object into the Optional.
    /// @param to_copy The object to copy
    constexpr Optional(meta::copy_trivial_t<const T&> to_copy)
    ...
    constexpr Optional(meta::InPlaceT, Args&&... args)
    ...
    constexpr Optional& operator=(meta::NoneT)
      noexcept(std::is_nothrow_destructible_v<T>)
    {
      reset();
    }

...
namespace clt
{
  ...
  public:
    ...
    constexpr std::pair<T*, InsertionResult> insert(meta::copy_trivial_t<const T&> key)
  ...
  private:
    ...
    static constexpr bool find_key(size_t key_hash, meta::copy_trivial_t<const T&> key, size_t& prob,
      const Vector<details::KeySentinel>& metadata, Slot* ptr, size_t capacity) noexcept
    {
      assert_true("Ensure hash match!", key_hash == GetHash(key));
...
struct fmt::formatter<clt::StableSet<T, PER_NODE, ALLOCATOR>>
  : fmt::formatter<clt::FlatList<T, PER_NODE, ALLOCATOR>>
{
  template<typename FormatContext>
  auto format(const clt::StableSet<T, PER_NODE, ALLOCATOR>& vec, FormatContext& ctx)
  {
    return fmt::formatter<clt::FlatList<T, PER_NODE, ALLOCATOR>>::format(vec.internal_list(), ctx);
  }
};
